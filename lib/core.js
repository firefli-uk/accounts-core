...
AT.prototype.configure = function(config) {
  ...
  const options = _.omit(config, "texts", "reCaptcha");
  ...
  if (config.texts) {
    ...
    const simpleTexts = _.omit(texts, "button", "errors", "info", "inputIcons", "socialIcons", "title");
...
AT.prototype.getField = function(fieldId) {
  const field = _.filter(this._fields, function(field) {
...
AT.prototype.oauthServices = function() {
  ...
  if (Accounts.loginServiceConfiguration) {
    const services = Accounts.loginServiceConfiguration.find();
    configuredServices = _.pluck(services.fetch(), "service");
  }

  // Builds a list of objects containing service name as _id and its configuration status
  const services = _.map(names, function(name) {
    return {
      _id : name,
  ...
  const showUnconfigured = typeof Accounts._loginButtonsSession !== "undefined";
...
AT.prototype.removeField = function(fieldId) {
  ...
  const index = _.indexOf(_.pluck(this._fields, "_id"), fieldId);
