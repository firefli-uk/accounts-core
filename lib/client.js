...
AT.prototype.oauthServices = function() {
  ...
  if (ServiceConfiguration.configurations) {
    configuredServices = ServiceConfiguration.configurations.find().map(config => config.service);
  }
...
AT.prototype._init = function() {
  ...
  const gContinuousValidation = !!AccountsTemplates.options.continuousValidation;
  ...
  if (this.options.showReCaptcha) {
    ...
    if (atSecretKey || settingsSecretKey) {
      ...
      const loc = atSecretKey ? "User Accounts configuration!" : "Meteor settings!";
