...
AT.prototype.oauthServices = async function() {
  ...
  if (ServiceConfiguration.configurations) {
    configuredServices = await cursor.mapAsync(config => config.service);
  }
...
AT.prototype._init = function() {
  ...
  if (this.options.showAddRemoveServices) {
    ...
    Meteor.publish("userRegisteredServices", function() {
      ...
      /*
      if (userId) {
        var user = await Meteor.users.findOne(userId);
  ...
  if (this.options.sendVerificationEmail && this.options.enforceEmailVerification) {
    Accounts.validateLoginAttempt(function(attempt) {
      ...
      if (loginEmail) {
        const email = _.filter(user.emails, function(obj) {
        ...
        const emailVerified = _.chain(user.emails)
        .pluck('verified')
        .any()
        .value();
  ...
  if (this.options.showReCaptcha) {
    const atSecretKey = AccountsTemplates.options.reCaptcha && AccountsTemplates.options.reCaptcha.secretKey;
    const settingsSecretKey = Meteor.settings.reCaptcha && Meteor.settings.reCaptcha.secretKey;
